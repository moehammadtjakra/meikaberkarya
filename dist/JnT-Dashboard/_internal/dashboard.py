...
with tab1:
    ...
    cap.caption("Default dari histori tiap produk. Edit nilai, **tambah baris** produk baru "
                "(klik + di bawah tabel), lalu simulasi otomatis menyesuaikan.")
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "Resi Historis": st.column_config.NumberColumn("Resi Histori", disabled=True,
                                                           help="Jumlah resi historis (acuan)."),
            "Budget/Hari": st.column_config.NumberColumn("Budget/Hari (Rp)", min_value=0,
                                                         step=10000, format="%d"),
            "CPL": st.column_config.NumberColumn("CPL (Rp)", min_value=0, step=500, format="%d"),
            "Nilai Produk": st.column_config.NumberColumn("Nilai Produk (Rp)", min_value=0,
                                                          step=1000, format="%d"),
            "HPP": st.column_config.NumberColumn("HPP (Rp)", min_value=0, step=1000, format="%d"),
        })
    ...
    overrides = dict(closing_rate=closing, success_rate=success, ongkir_per_resi=ongkir,
                     ...
                     pct_cod=pct_cod / 100, horizon_days=horizon, mode=mode, daily_lag=lag)
    sim = ce.simulate_multi(baseline, recv_dist, edited, overrides)
    s = sim["summary"]

    st.caption(
        f"🧮 **Net real / resi** = (Nilai Produk − HPP) + Cashback ({rp(cashback_pct/100*ongkir)}) "
        f"− COD Fee  →  rata² **{rp(s['avg_net_per_resi'])}**/resi.  "
    ...
    kpi(r1[1], "Estimasi Lead", num(s["n_lead"]))
    kpi(r1[2], "Estimasi Order", num(s["n_order"]))
    kpi(r1[3], "Estimasi Resi", num(s["n_resi"]),
        f"{num(s['n_sukses'])} berhasil ({fmt.persen(s['success_rate']*100,0)})")
    r2 = st.columns(4)
    kpi(r2[0], "Total COD", rp(s["nilai_cod"]), cls="green")
    kpi(r2[1], "Total Transfer", rp(s["nilai_transfer"]), cls="green")
    kpi(r2[2], "Cashback (Omzet)", rp(s["total_cashback"]), cls="green")
    kpi(r2[3], "Net Real (horizon)", rp(s["total_net"]),
        f"{rp(s['avg_net_per_resi'])}/resi", cls="green")
    st.markdown("##### 💵 Kebutuhan Modal & Pencairan")
    r3 = st.columns(4)
    kpi(r3[0], "Modal HPP Produk", rp(s["modal_hpp"]),
        "stok barang yg harus dibeli", cls="amber")
    kpi(r3[1], "Modal Kerja (talangan)", rp(s["modal_kerja"]),
        "tutup jeda iklan↔pencairan", cls="amber")
    kpi(r3[2], "Total Modal Dibutuhkan", rp(s["modal_total"]),
        "Modal HPP + Modal Kerja", cls="amber")
    kpi(r3[3], "Outstanding COD (puncak)", rp(s["outstanding_peak"]), cls="amber")
    r4 = st.columns(4)
    kpi(r4[0], "Dana Cair Minggu Ini", rp(s["cair_minggu_ini"]), cls="green")
    kpi(r4[1], "Dana Cair Bulan Ini", rp(s["cair_bulan_ini"]), cls="green")
    kpi(r4[2], "Cashflow Bersih (horizon)", rp(s["net_cashflow_total"]),
        f"selama {s['horizon_days']} hari", cls="green")
    kpi(r4[3], "Avg Durasi Kirim", f"{(s['avg_durasi'] or 0):.1f} hari")

    # ---------- HASIL PER PRODUK ----------
    pp_df = sim["per_product"].copy()
    if not pp_df.empty:
        with st.expander("📦 Hasil Simulasi per Produk (lead, resi, net, modal, ROI)", expanded=True):
            show = pd.DataFrame({
                "Produk": pp_df["Produk"],
                "Budget/Hari": pp_df["budget_harian"].map(rp),
                "Lead": pp_df["lead"].map(num),
                "Resi": pp_df["resi"].map(num),
                "Net/Resi": pp_df["net_per_resi"].map(rp),
                "Net Total": pp_df["net_total"].map(rp),
                "Modal HPP": pp_df["modal_hpp"].map(rp),
                "ROI": (pp_df["roi"] * 100).round(0).map(lambda v: fmt.persen(v, 0)),
            })
