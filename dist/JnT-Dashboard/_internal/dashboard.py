...
try:
    ...
    st.info("Jika memakai Google Sheet: cek GSHEET_WEBAPP_URL & GSHEET_TOKEN "
            "(config.py / secrets_local.py) dan pastikan deployment Web App = 'Anyone'.")
    st.stop()
...
with st.sidebar:
    ...
    if _use_gsheet():
        st.success("Sumber: 🌐 Google Sheet (live)")
        st.caption(f"{len(df_all):,} resi".replace(",", "."))
    else:
        st.success(f"Data: {os.path.basename(str(xlpath))}")
